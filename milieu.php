...
class curl {
	...
	public function request ($method, $url, $param, $header) {
        ...
        if(is_numeric(strpos($url, 'disposablemail.com'))) {
            curl_setopt($this->ch, CURLOPT_COOKIEJAR,'cookie_disposablemail.txt');
		    curl_setopt($this->ch, CURLOPT_COOKIEFILE,'cookie_disposablemail.txt');
        }
    ...
    case '1':
        ...
        while($no <= $_qty) {
            ...
            foreach ($randomuser as $value) {
                ...
                new_email:
                $email = $milieu->new_email($user_email);
                if($email == FALSE) {
                    echo "[!] ".date('H:i:s')." | Gagal membuat email, santuy..\n";
                    sleep(1);
                    goto new_email;
                }
                ...
                if($regis == FALSE) { 
                    ...
                    if($_referal == FALSE) { 
                        ...
                        if($akun_utama == TRUE) {
                            echo "[i] ".date('H:i:s')." | Registrasi Berhasil id-".$regis_id." [email:".$email."+pass:".$pass."+reff:".$referral."]\n";
                        } else {
                            echo "[i] ".date('H:i:s')." | Registrasi Berhasil id-".$regis_id." [email:".$email."-pass:".$pass."]\n";
                        } 
                        // cek inbox
                        echo "[i] ".date('H:i:s')." | Mengecek Email...\n";
                        ...
                        if($inbox == FALSE) {
                            ...
                            if($ib<=20) {
                                ...
                                echo "[!] ".date('H:i:s')." | Skip..Link aktivasi tidak ditemukan\n\n";
                            }
                        } else {                   
                            //aktivasi akun
                            $_activation = $milieu->activation($inbox, $bearer, $header);
                            if($_activation == TRUE) {
                                echo "[".$no++."] ".date('H:i:s')." | Aktivasi email berhasil\n\n";
                                ...
                                echo "[i] ".date('H:i:s')." | Aktivasi email gagal, santuy..\n\n";
                            }
                        }
                    }
                }        
